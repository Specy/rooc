...
utilizza: quali costrutti mette a disposizione e quale ne è il significato. La
realizzazione interna di tali costrutti, ossia il modo in cui vengono analizzati
e tradotti dal compilatore, è rimandata ai capitoli successivi. Per tutti gli
esempi si fa riferimento alla sintassi effettivamente accettata dalla grammatica
del linguaggio (Capitolo~\ref{cap:parsing}).
...
vengono tipicamente forniti i dati della specifica istanza, mantenendoli
...
esempi che verranno richiamati nel resto della tesi: il problema dello zaino e
...
un costo, indicato dopo il carattere \texttt{:}, come nel Listato~\ref{lst:grafo}.
...
Quando un iteratore produce tuple, è possibile decomporne le componenti
direttamente nella dichiarazione, come in \texttt{(value, i) in
enumerate(values)}: a ogni iterazione \texttt{value} assume il valore
dell'elemento e \texttt{i} il suo indice. Il simbolo \texttt{\_} indica una
componente da ignorare, come in \texttt{(\_, u) in neigh\_edges(v)}, dove
...
stringhe ne produce la concatenazione. Le operazioni insiemistiche tra
collezioni non sono espresse tramite operatori, ma tramite le funzioni
