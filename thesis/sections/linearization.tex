...
\texttt{LinearConstraint} (Listato~\ref{lst:linearconstraint}).

\begin{lstlisting}[language=rust, caption={La struttura \texttt{LinearConstraint} (campi principali).}, label={lst:linearconstraint}]
...
Il valore assoluto è un ulteriore costrutto che si presterebbe a una
riformulazione lineare analoga. Allo stato attuale, però, la sua linearizzazione
non è implementata: incontrando un'espressione di valore assoluto, il
compilatore segnala un errore di costrutto non supportato. Il suo trattamento è
quindi rimandato agli sviluppi futuri (Capitolo~\ref{cap:conclusioni}).
...
Il risultato è uno \texttt{StandardLinearModel} (Listato~\ref{lst:standardmodel}),
...
\begin{lstlisting}[language=rust, caption={La struttura \texttt{StandardLinearModel}.}, label={lst:standardmodel}]
struct StandardLinearModel {
